...
- **Dynamic Model Extensions**: New API `Hook::model(Class::class)` to extend Eloquent models without modifying their source code.
- **Dynamic Relations**: Support for adding relationships on the fly using `relation()`.
- **Dynamic Scopes**: Support for adding instance scopes via `scope()` and global scopes via `globalScope()`.
- **Model Events**: Support for class-level observers using `on('event', callback)`.
- **Dynamic Casts**: Support for injecting property casts at runtime via `cast()`.
- **Accessors & Mutators**: Support for adding virtual attributes via `attribute()` and simple accessors via `accessor()`.
- **Dynamic Policy Fixes**: Support for dynamically adding `fillable`, `guarded`, `hidden`, and `visible` attributes via `Hook::model()`.
- **HasDynamicHooks Trait**: A new trait that enables even deeper model extensibility:
  - **Dynamic Methods**: Add any method to a model using `method()`.
  - **Property Filtering**: Intercept every property access via `model.{name}.attribute.{key}` filters.
  - **Mutation Filtering**: Intercept property assignments via `model.{name}.set_attribute.{key}` filters.
  - **Serialization Filtering**: Intercept `toArray()` output via `model.{name}.to_array` filters.
- **Improved Roadmap Features**:
    - **Macroable Attributes**: PHP 8 attribute support (`#[DynamicRelation]`, `#[DynamicCast]`, `#[DynamicScope]`, `#[DynamicAccessor]`, `#[DynamicMutator]`) for declarative model extensions.
    - **Blade Fallbacks**: Support for default/fallback content in `@hook` directive.
    - **Circuit Breaker**: Automatic silencing of hooks that fail repeatedly (configurable threshold).
    - **Type-Safe DTOs**: Support for class name type-checks in hook signatures for DTO patterns.
    - **Performance Snapshots**: Improved metrics in Debugbar and Pulse, including hit counts and max durations.
    - **Enhanced Auto-Docs**: The `hook:generate-docs` command now discovers multi-line hook calls and Blade directives.
- Added comprehensive testing for all new dynamic model features and roadmap items.
